...
def get_market_time(name, df, input_time):
  ...
  for i in week:
    if df[i][ind[0]] == '24시간 영업':
      ...
      if time[0][:2] == '오전':
        start=datetime.strptime(time[0], '오전 %H:%M').replace(year=input_time.year, month=input_time.month, day=input_time.day)
      else :
        start=datetime.strptime(time[0], '오후 %H:%M').replace(year=input_time.year, month=input_time.month, day=input_time.day) + dt.timedelta(hours=12)
      if time[1][:2] == '오전':
        close=datetime.strptime(time[1], '오전 %H:%M').replace(year=input_time.year, month=input_time.month, day=input_time.day)
      else :
        close=datetime.strptime(time[1], '오후 %H:%M').replace(year=input_time.year, month=input_time.month, day=input_time.day) + dt.timedelta(hours=12)
...
def attraction_route_recommend(input = '', input_time = '', finish_times = '', Osaka_time_path = '', User_df_path = '', total_Osaka_path = ''):
